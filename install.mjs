 ...
 * 
 * Usage:
 *   npx pi-cohort          # Install to ~/.pi/agent/extensions/subagent
 *   npx pi-cohort --remove # Remove the extension
 */
...
const REPO_URL = "https://github.com/jjuraszek/pi-cohort.git";
...
if (isHelp) {
	console.log(`
pi-cohort - Pi extension for delegating tasks to subagents

Usage:
  npx pi-cohort          Install the extension
  npx pi-cohort --remove Remove the extension
  npx pi-cohort --help   Show this help
...
if (isRemove) {
	if (fs.existsSync(EXTENSION_DIR)) {
		...
		console.log("pi-cohort removed");
	} else {
		console.log("pi-cohort is not installed");
	}
	process.exit(0);
}

// Install
console.log("Installing pi-cohort...\n");
...
if (fs.existsSync(EXTENSION_DIR)) {
	...
	if (isGitRepo) {
		...
		try {
			...
			console.error("  npx pi-cohort --remove && npx pi-cohort");
			process.exit(1);
		}
	} else {
		console.log(`Directory exists but is not a git repo: ${EXTENSION_DIR}`);
		console.log("Remove it first with: npx pi-cohort --remove");
		process.exit(1);
	}
} else {
	// Fresh install
