...
After a worktree parallel step completes, per-agent diff stats are appended to the output and full patch files are written to a run-scoped directory (`<session artifacts>/<runId>/worktree-diffs`), so concurrent or sequential worktree dispatches from the same session never overwrite each other's patches. Background keeps `<asyncDir>/worktree-diffs/step-<i>` and chain keeps `<chainDir>/worktree-diffs/step-<i>` - both already run-scoped. The diff is captured before every post-execution return - normal, interrupted, detached, or intercom-receipt - so the summary is attached even when a run ends early; a capture failure is reported in the summary rather than showing up as an empty patch. Worktrees and temp branches are still cleaned up in `finally` blocks.

A relative `output:` on a `worktree: true` task no longer resolves inside the throwaway checkout - it resolves to the run's per-task directory instead (`<session artifacts>/<runId>/task-<i>/` foreground, `<asyncDir>/step-<i>/task-<j>/` async), so the report survives worktree teardown and stays out of the captured patch. An absolute `output:` is used as-is. A relative path that escapes its per-task directory (e.g. `../report.md`) or normalizes to the directory itself (`.`) is rejected at dispatch. `reads:` and the task's working directory still resolve inside the checkout, unchanged.
