...
After a worktree parallel step completes, per-agent diff stats are appended to the output and full patch files are written to a run-scoped directory (`<session artifacts>/<runId>/worktree-diffs`), so concurrent or sequential worktree dispatches from the same session never overwrite each other's patches. Background keeps `<asyncDir>/worktree-diffs/step-<i>` and chain keeps `<chainDir>/worktree-diffs/step-<i>` - both already run-scoped. The diff is captured before every post-execution return - normal or interrupted - so the summary is attached even when a run ends early; a capture failure is reported in the summary rather than showing up as an empty patch. Worktrees and temp branches are still cleaned up in `finally` blocks.
