...
| `## Repo state` | one field per line: `toplevel`, `worktree: yes <path>` or `worktree: no`, `branch` (`detached` when none), `HEAD`, `base`, `dirty: <git status --porcelain>` or `dirty: clean`, `diff-stat`, `test cmd`; `base: unknown` when no remote resolves (then `diff-stat: unavailable`); any field `unavailable` when its command failed; `## Repo state: not a git repo` when so | always |
