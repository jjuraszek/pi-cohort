...
import { resolveSubagentResultStatus } from "../shared/result-children.ts";
...
function rememberForegroundRun(state: SubagentState, input: { runId: string; mode: "single" | "parallel" | "chain"; cwd: string; results: SingleResult[] }): void {
	...
	state.foregroundRuns.set(input.runId, {
		...
		children: input.results.map((result, index) => ({
			agent: result.agent,
			index,
			status: resolveSubagentResultStatus({ exitCode: result.exitCode, interrupted: result.interrupted }),
...
async function resumeAsyncRun(input: {
	...
	const agentConfig = discoveredAgents.find((agent) => agent.name === target.agent);
	if (!agentConfig) {
		return {
...
function createForegroundControlNotifier(data: Pick<ExecutionContextData, "controlConfig">, deps: Pick<ExecutorDeps, "pi">): (event: ControlEvent) => void {
	return (event) => emitControlNotification({ pi: deps.pi, controlConfig: data.controlConfig, event });
}
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	if (!isAsyncAvailable()) {
		return {
			content: [{ type: "text", text: "Async mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the pi-cohort package dependencies are installed." }],
			isError: true,
			details: { mode: "single" as const, results: [] },
		};
	}
	...
	if (hasTasks && params.tasks) {
		...
		const parallelTasks = params.tasks.map((task, index) => ({
			...
			output: normalizeTopLevelOutput(task.output, agentConfigs[index]?.output),
	...
	if (hasSingle) {
		...
		const effectiveOutput = normalizeTopLevelOutput(params.output, a.output);
...
async function runChainPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	if (chainResult.requestedAsync) {
		if (!isAsyncAvailable()) {
			return {
				content: [{ type: "text", text: "Background mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the pi-cohort package dependencies are installed." }],
				isError: true,
				details: { mode: "chain" as const, results: [] },
			};
		}
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	let effectiveOutput = normalizeTopLevelOutput(params.output, agentConfig.output);
	const effectiveOutputMode = params.outputMode ?? "inline";
	const currentMaxSubagentDepth = resolveCurrentMaxSubagentDepth(deps.config.maxSubagentDepth);
	...
	if (params.clarify === true && ctx.hasUI) {
		...
		if (override?.output !== undefined) effectiveOutput = normalizeTopLevelOutput(override.output, agentConfig.output);
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		const allowClarifyTaskPrompt = hasChain && effectiveParams.clarify === true && ctx.hasUI && !(effectiveParams.chain?.some(isParallelStep) ?? false);
