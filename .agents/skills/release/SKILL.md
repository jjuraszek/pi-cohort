---
name: release
description: Use when asked to release, publish, bump the version, or cut a tag for jjuraszek/pi-cohort.
---

# Release

`pi-cohort` publishes to **npm** (public, unscoped); the `pi-package` keyword
lists it on `https://pi.dev/packages/pi-cohort`. Users install with
...
| Level | When |
|---|---|
| `patch` | fixes, prose, internal changes that don't alter dispatch shape |
| `minor` | new agent, chain, tool surface, or backward-compatible feature |
| `major` | breaking change: agent/tool rename, config-schema break, removed behavior |
...
bash .agents/skills/release/scripts/release.sh verify 6.0.2
```

Watches the release workflow to a terminal state, polls
`npm view pi-cohort@X.Y.Z version` until live, then checks the pi.dev catalog.
...
pins (`npm:pi-cohort@<old>`) are bumped; git-tag pins and stale `pi-subagents`
...
- reported success without `npm view pi-cohort@X.Y.Z` printing the version
...
Register `pi-cohort` as a trusted publisher on npmjs.com: Settings -> Trusted
Publishing -> GitHub Actions publisher for repo `jjuraszek/pi-cohort`, workflow
